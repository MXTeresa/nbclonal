...
#' @param maxFS Maximum number of final sizes being calculated, equal to or smaller than maxMuGen
...
#' list_clonal(seq(2, 10, by = 2), 0.8, seq(1.2, 2.5, by = 0.1), 60, 50, 7)
#' @export
...
#' Clonal Mutations with Various Parameters and Mu Fixed
#'
#' This function calculates for the clonal mutation distribution
#' across pairs of parameters N and R0.
#'
#' @param n_values A sequence of initial population sizes
...
#' @return A list of clonal mutation pmf with different parameters
#' @examples
#' list_clonal_mu(1:10, 2:5, 0.3, 50, 50, 5)
#' list_clonal_mu(seq(2, 10, by = 2), seq(2, 4, by = 0.3), 2.1, 60, 50, 7)
#' @export
...
#' pmf.clonal_params(2, listClonal, seq(2, 10, by = 2), 0.8, seq(1.2, 2.5, by = 0.1), 60, 50)
#' @export
...
#' Clonal Mutation Probability with Various Parameters and Mu Fixed
#'
#' This function calculates the probability of getting a certain
#' number of clonal mutations across pairs of parameters N and R0.
#'
#' @param clonal The number of clonal mutations being calculated, a positive integer
#' @param listClonal Returned list from function list_clonal_mu with same parameters
#' @param n_values A sequence of initial population sizes
#' @param R0_values A sequence of reproduction number
#' @param mu Mutation rate, a positive number
#' @param maxMuGen Maximum number of mutant lineages being calculated
#' @param maxFS Maximum number of final sizes being calculated, equal to or smaller than maxMuGen
#' @return A data frame of N, R0 values and corresponding probability of getting the input clonal mutations
#' @examples
#' pmf.clonal_params_mu(0, listClonal, 1:10, 2:5, 0.3, 50, 50)
#' pmf.clonal_params_mu(2, listClonal, seq(2, 10, by = 2), seq(2, 4, by = 0.3), 1.2, 60, 50)
#' @export
...
#' Initial Population Size Distribution as Adjusted Poisson
#'
#' This function calculates the poisson distribution of initial population size
#' N given a mean of lambda and also the adjusted distribution normalized by at
...
#' Initial Population Size Distribution as Adjusted Negative Binomial
#'
#' This function calculates the negative binomial distribution of initial
#' population size N given a mean of lambda and also the adjusted distribution
#' normalized by at least one of the initial viral particles established.
#'
#' @param lambda Mean of Poisson distribution, a positive number
#' @param R0 Reproduction number, a positive number
#' @param maxIni Maximum initial population size being calculated
#' @param k dispersion paramter of negative binomial distribution
#' @return A data frame of N, distribution type and corresponding probability of getting that number of initial population size
#' @examples
#' pmf.ini_size_nb(2.1, 1.6, 10, 0.1)
#' pmf.ini_size_nb(0.8, 2.4, 6, 0.5)
#' @export
...
#' points in the provided data set given a certain lambda, R0, and mu.
#'
...
#' LL_lambda(flu_data, listClonal, 0.2, 10, 0.7, 60, 50, 8)
#' @export
...
#' Lambda Log Likelihood with Negative Binomial as Initial Distribution
#'
#' This function calculates the log likelihood of getting the observed data
#' points in the provided data set given a certain lambda, R0, and mu.
#'
...
#' @return A number representing the probability
#' @examples
#' LL_lambda_nb(sample_data, listClonal, 2.1, 1.6, 0.4, 50, 50, 10, 0.1)
#' LL_lambda_nb(flu_data, listClonal, 0.2, 10, 0.7, 60, 50, 8, 0.5)
#' @export
...
#' Lambda Log Likelihood with Mu Fixed
#'
#' This function calculates the log likelihood of getting the observed data
#' points in the provided data set given a certain lambda, R0, and mu.
#'
...
#' @return A number representing the probability
#' @examples
#' LL_lambda_mu(sample_data, listClonal, 2.1, 1.6, 0.4, 50, 50, 10)
#' LL_lambda_mu(flu_data, listClonal, 0.2, 10, 0.7, 60, 50, 8)
#' @export
...
#' Proportion Log Likelihood
#'
#' This function calculates the log likelihood of getting the observed data
#' points in the provided data set given a certain proportion of N=1, R0, and
#' mu. We usually assume that R0 is a known value and aims to estimate lambda
#' and mu.
#'
#' @param df Data set with first column named "ClonalMu" recording number of clonal mutations, and the second column named "freq" recording number of times the clonal mutation is observed
#' @param listClonal Returned list from function list_clonal with same parameters
#' @param prop Proportion of N=1, between 0 and 1
#' @param R0 Reproduction number, a positive number
#' @param mu Mutation rate, a positive number
#' @param maxMuGen Maximum number of mutant lineages being calculated
#' @param maxFS Maximum number of final sizes being calculated, equal to or smaller than maxMuGen
...
#' @return A number representing the probability
#' @examples
#' LL_prop(sample_data, listClonal, 0.7, 1.6, 0.4, 50, 50, 10)
#' LL_prop(flu_data, listClonal, 0.2, 10, 0.7, 60, 50, 8)
#' @export
...
#' LL_meanNb.df(flu_data, listClonal, seq(0.1, 1.4, by = 0.1), 10, seq(1.2, 2.5, by = 0.1), 60, 50, 8)
#' @export
...
#' Log Likelihood Heat Map with Negative Binomial as Initial Distribution
#'
#' This function calculates the log likelihood of getting the observed data
#' points in the provided data set across a set of parameters lambda, mean N,
#' transmission bottleneck Nb, and mu. Since the actual distribution of initial
...
#' Log Likelihood Heat Map with Mu Fixed
#'
#' This function calculates the log likelihood of getting the observed data
#' points in the provided data set across a set of parameters lambda, mean N,
#' transmission bottleneck Nb, and R0. Since the actual distribution of initial
#' population size N does not follow a perfect Poisson distribution but an
#' altered version of Poisson distribution, we are also interested in estimating
#' the most likely mean N, which might be slightly different from lambda. We can
#' also estimate the mean of transmission bottleneck size Nb by applying a
...
#' @param maxFS Maximum number of final sizes being calculated, equal to or smaller than maxMuGen
#' @param maxIni Maximum initial population size being calculated
#' @return A data frame with lambda, mean N, mean Nb, R0, and corresponding probabilities of getting the set of parameters given data points in the input data set
#' @examples
#' LL_meanNb_mu.df(sample_data, listClonal, seq(0.1, 7, by = 0.2), 2:5, 0.3, 50, 50, 10)
#' LL_meanNb_mu.df(flu_data, listClonal, seq(0.1, 1.4, by = 0.1), seq(1.2, 2.5, by = 0.1), 1.2, 60, 50, 8)
#' @export
...
#' Log Likelihood Heat Map for Proportion
#'
#' This function calculates the log likelihood of getting the observed data
#' points in the provided data set across a set of parameters proportion of N=1,
#' mean N, and mu.
#'
#' @param df Data set with first column named "ClonalMu" recording number of clonal mutations, and the second column named "freq" recording number of times the clonal mutation is observed
#' @param listClonal Returned list from function list_clonal with same parameters
#' @param prop_values A sequence of proportion of N=1, between 0 and 1
#' @param R0 Reproduction number, a positive number
#' @param mu_values A sequence of mutation rate
#' @param maxMuGen Maximum number of mutant lineages being calculated
#' @param maxFS Maximum number of final sizes being calculated, equal to or smaller than maxMuGen
#' @param maxIni Maximum initial population size being calculated
#' @return A data frame with proportion, mean N, mu, and corresponding probabilities of getting the set of parameters given data points in the input data set
#' @examples
#' LL_prop.df(sample_data, listClonal, seq(0.1, 1, by = 0.2), 1.6, seq(0.1, 2, by = 0.2), 50, 50, 10)
#' LL_prop.df(flu_data, listClonal, seq(0.1, 0.8, by = 0.1), 10, seq(1.2, 2.5, by = 0.1), 60, 50, 8)
#' @export
LL_prop.df <- function(df, listClonal, prop_values, R0, mu_values, maxMuGen, maxFS, maxIni) {
  prob <- 1:length(mu_values) %>%
    mclapply(function(x) {
...
#' meanClonal(3, 3.5, 1.6, 120, 110, 5)
#' @export
...
#' meanClonal.df(3, seq(1.05, 3, by = 0.05), seq(0.3, 0.5, by = 0.1), 120, 110, 5)
#' @export
