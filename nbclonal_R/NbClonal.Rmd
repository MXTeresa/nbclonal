...
ggplot(df_pois, aes(N, prob)) +
  geom_col(aes(fill = type), position = "dodge") +
  scale_fill_discrete_sequential(palette = "Mint", nmax = 5, order = c(4, 2)) +
  labs(
    x = "N",
    y = "probability",
...
ggplot(df_meanNbLL) +
  geom_contour_fill(aes(x = lambda, y = mu, z = prob), na.fill = TRUE, bins = 400) +
  ...
  scale_fill_continuous_sequential(palette = "Mint", rev = FALSE, 
                                   na.value = "transparent", limits = c(-85.9,-82.9)) +
...
$$
\bar{N} = \sum_{n=1}^{\infty} n P(N=n;\lambda)
$$
...
\begin{align*}
P(N_b = k)
& = \sum_{n=k}^{\infty} P(N=n;\lambda) \times Binom(k;n,1-\frac{1}{R_0}) / [1-Binom(0;n,1-\frac{1}{R_0})] \\
& = \sum_{n=k}^{\infty} \frac{Poisson(N=n;\lambda) \times (1-\frac{1}{R_0^n})}{\sum P(N=n; \lambda)} \times \binom{n}{k} (1-\frac{1}{R_0})^k (\frac{1}{R_0})^{n-k} / [1-\binom{n}{0}  (\frac{1}{R_0})^n]
\end{align*}
```
...
where $r=0.01$ and $p=\frac{r}{r+mean}$. We will still use $\lambda$ to denote the mean of the negative binomial distribution.
...
```{r, echo = FALSE}
ggplot(df_lambdaLL_nb) +
  geom_contour_fill(aes(x = lambda, y = mu, z = prob), na.fill = TRUE, bins = 600) +
  geom_hline(aes(yintercept = 0.4), linetype = "dashed") +
  geom_vline(aes(xintercept = 2.1), linetype = "dashed") +
  scale_fill_continuous_sequential(palette = "Mint", rev = FALSE, 
                                   na.value = "transparent", limits = c(-85.9,-82.9)) +
...
```{r, echo = FALSE}
ggplot(df_prop_nb) +
  geom_contour_fill(aes(x = prop, y = mu, z = prob), na.fill = TRUE, bins = 100) +
  geom_hline(aes(yintercept = 0.4), linetype = "dashed") +
  scale_fill_continuous_sequential(palette = "Mint", rev = FALSE, 
                                   na.value = "transparent", limits = c(-86,-83)) +
  labs(x = "Proportion of N = 1", 
       y = expression(mu), 
       fill = "",
       title = "Log Likelihood Plot of Observing the Sample Data with Given Parameters") +
  scale_x_continuous(limits = c(0.95, 1), breaks = seq(0, 1, 0.01))
```
...
```{r, echo = FALSE}
ggplot(df_P_trans, aes(N, prob)) +
  geom_point(color = "#44948C") +
  geom_line(color = "#44948C") +
  labs(y = "P(transmitted & fixed)", title = "Probability of an Allele is Transmitted and Fixed") +
